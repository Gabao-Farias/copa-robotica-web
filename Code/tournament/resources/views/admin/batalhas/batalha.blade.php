	...
	<div class="wrapper-md">
		<div class="row less-gutter">
			<div class="col-lg-10 col-lg-offset-1 col-md-12">
				@include('admin.partials.round-template', ['batalhas' => [$batalha]])

				<input type="hidden" id="round_data" value="{{ $round }}">
				<div class="row">
					<div class="col-xs-4">
                        <div class="buttons-set" data-equipe-id="{{ $batalha->equipe1->id }}" data-equipe-nome="{{ $batalha->equipe1->nome }}">
                            @include('admin.batalhas.buttons-set')
                        </div>
					...
					<div class="col-xs-4">
                        <h2 style="font-weight: bold; margin-top: 0; font-size: 25px;">Round {{ $round->ordem }} <span class="round-status">({{ Helpers::resolverStatusRound($round->status) }})</span></h2>
						<div class="partida-control">
							<span class="glyphicon {{ $controlIcon }} control" data-state="{{ $controlValue }}"></span>
						</div>
					...
					<div class="col-xs-4">
                        <div class="buttons-set" data-equipe-id="{{ $batalha->equipe2->id }}" data-equipe-nome="{{ $batalha->equipe2->nome }}">
						    @include('admin.batalhas.buttons-set')
                        </div>
...
@section('assets')
	...
	<script>
		$(function() {
			new Round($('#round_data').val(), false);
		});
	</script>
@endsection